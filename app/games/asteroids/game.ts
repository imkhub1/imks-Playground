...
// Canvas resolution (fixed; CSS scales to fit the bezel).
const W = 800;
const H = 600;
...
export function createAsteroids(gameCtx: GameContext): GameController {
  ...
  const justPressed: Partial<Record<string, boolean>> = {};
  ...
  // ── Classes (defined inside closure to capture ctx / keys / W / H) ─────────
  ...
  class Ship {
    ...
    constructor() {
      this.tripleShot = 0;
      // Explicitly initialize all properties so strict mode is satisfied;
      // reset() sets their real values immediately after.
      this.x = W / 2;
      this.y = H / 2;
  ...
  // ── State emission (batched: only fires when something changes) ────────────
  let lastEmitted: GameState | null = null;
  let gameOverEmitted = false;

  function emitState() {
    const s: GameState = { score, lives, level, status: state };
    if (
      !lastEmitted ||
      lastEmitted.score !== s.score ||
      lastEmitted.lives !== s.lives ||
      lastEmitted.level !== s.level ||
      lastEmitted.status !== s.status
    ) {
      lastEmitted = s;
      gameCtx.onState(s);
    }
  ...
  function update(dt: number) {
    if (state === "gameover") {
      // Restart is handled by React's GameOverModal via controller.restart()
    ...
    for (const b of bullets) {
      for (const a of asteroids) {
        if (!a.dead && !b.dead && dist(b, a) < a.radius) {
          ...
          if (!powerUpSpawned) {
            killsSinceSpawn++;
            if (killsSinceSpawn >= 5 || Math.random() < POWERUP_DROP_CHANCE) {
  ...
  function draw() {
    ...
    if (state === "gameover")
      drawOverlay("GAME OVER", `SCORE: ${score}   —   PRESS PLAY AGAIN`);
  }
  ...
  return {
    ...
    resume() {
      paused = false;
      lastTime = null; // avoid dt spike after a long pause
    },
    restart() {
